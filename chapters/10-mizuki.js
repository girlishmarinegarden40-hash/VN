window.AliceCourtyardChapterFiles = window.AliceCourtyardChapterFiles || [];
window.AliceCourtyardChapterFiles.push({
  id: "mizuki",
  title: "第十一章",
  subtitle: "Mizuki",
  location: "门边",
  lines: [
    {
      type: "card",
      eyebrow: "Chapter 11",
      title: "Mizuki",
      subtitle: "这一次，我必须先看见。",
      location: "门边",
      background: "./assets/bg/bg_garden_night.webp",
      bgm: "./assets/bgm/bgm_mirror_relive.mp3",
      bgmName: "镜中回声",
      cardDuration: 2450,
      pauseAfter: 560
    },
    {
      location: "庭院门边",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_001_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      cg: "./assets/cg/cg_mizuki_at_the_door.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp" },
        { slot: "right", src: "./assets/sprites/a017/a017_neutral.webp", dim: true }
      ],
      text: "Mizuki总站在门边。风大一点，她会看门。有人经过，她也会先扫过去一眼，像只要自己先看见，就还来得及阻止什么。"
    },
    {
      location: "庭院门边",
      speaker: "A-017",
      voice: "./assets/voice/10-mizuki/10-mizuki_002_a017.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp", dim: true },
        { slot: "right", src: "./assets/sprites/a017/a017_neutral.webp" }
      ],
      text: "你在等谁吗？"
    },
    {
      location: "庭院门边",
      speaker: "Mizuki",
      voice: "./assets/voice/10-mizuki/10-mizuki_003_mizuki.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp" },
        { slot: "right", src: "./assets/sprites/a017/a017_neutral.webp", dim: true }
      ],
      text: "我在看，还有没有人会被带走。"
    },
    {
      location: "庭院门边",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_003a_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp", dim: true },
        { slot: "right", src: "./assets/sprites/a017/a017_neutral.webp" }
      ],
      text: "远处，Yume 膝上册子翻页的声响极轻，却和 Mizuki 钉在门边的视线落在同一条线上——一个记下消失的位置，一个盯着会不会再有人被拖出门框。",
      pauseBefore: 180,
      pauseAfter: 220,
      textSpeed: 22
    },
    {
      location: "庭院门边",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_004_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp", dim: true },
        { slot: "right", src: "./assets/sprites/a017/a017_stunned.webp" }
      ],
      text: "A-017听见这句话时，没有立刻接上。因为她忽然明白，Mizuki不是在守门。她是在守那个自己曾经来不及看见的瞬间。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_005_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_dusk.webp",
      cg: "./assets/cg/cg_mizuki_dressed_by_younger_sister.webp",
      text: "在那之前，她其实并不擅长当姐姐。妹妹反而更像会照顾人的那一个。会替她把额前翘起的碎发压下去，会把她系得乱七八糟的领结重新拉正，还会笑着把刚拍好的贴纸往她袖口上贴。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_006_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_dusk.webp",
      text: "Mizuki总嫌麻烦，嫌妹妹像在把自己当成需要被整理的人。可她后来才明白，那就是妹妹一直在朝她伸手的方式。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_007_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "她们家那时在当地太显眼。不是什么传奇式的豪门，只是足够让人相信，两个女儿一旦被带走，就能很快换来一大笔钱。"
    },
    {
      location: "回忆",
      speaker: "绑匪",
      voice: "./assets/voice/10-mizuki/10-mizuki_008_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "姐妹都在，价会更高。东西拿到就走，别留活口。",
      pauseBefore: 180,
      pauseAfter: 240,
      textSpeed: 28
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_009_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "那句话落下去的时候，妹妹的手明显收紧了一下。Mizuki也是在那一刻才真正明白，留给她们的根本不是“等”，而是逃。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_010_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "她们试过往后门跑，试过把倒下的椅子拖到窗边，试过在脚步声换班的那几十秒里把门闩一点一点抬起来。可每一次，刚露出来的那一点缝隙都会很快重新合上。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_011_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      cg: "./assets/cg/cg_mizuki_shielded_by_younger_sister.webp",
      text: "直到最后一次，门真的被她们撞开了一条能让风灌进来的缝。妹妹先把她往外推了一把。也就是在那一刻，那个一直拉着她往前跑的人忽然回过身来，挡在了她前面。",
      pauseBefore: 220,
      pauseAfter: 260
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_012_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "Mizuki终于在那一刻扑了上去。她扑得太急，膝盖重重磕在地上，掌心蹭破，像一个迟到了太久的人，拼命想把“姐姐”这个词从已经发生的结果里抢回来。"
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_013_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      cg: "./assets/cg/cg_mizuki_last_failed_resistance.webp",
      text: "可她还是失败了。妹妹先死在她眼前，而她扑上去的时候，也只来得及把那个人影撞歪半步。接下来的那一下，终于落到她自己身上。她倒在妹妹身边，最后也死在了那一夜。",
      pauseBefore: 220,
      pauseAfter: 260,
      textSpeed: 22
    },
    {
      location: "回忆",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_014_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_void_white.webp",
      text: "她最后看见的，是妹妹还挡在她前面的那道影子。很薄，很小，却比那一夜所有的门和刀都更先一步把她护住。"
    },
    {
      location: "庭院门边",
      speaker: "Mizuki",
      voice: "./assets/voice/10-mizuki/10-mizuki_015_mizuki.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_shocked.webp" },
        { slot: "right", src: "./assets/sprites/a017/a017_stunned.webp", dim: true }
      ],
      text: "守不住。",
      pauseBefore: 180,
      textSpeed: 38
    },
    {
      location: "庭院门边",
      speaker: "Mizuki",
      voice: "./assets/voice/10-mizuki/10-mizuki_016_mizuki.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp" },
        { slot: "right", src: "./assets/sprites/a017/a017_stunned.webp", dim: true }
      ],
      text: "但如果还有下一个人路过这里，我至少能先看见。"
    },
    {
      location: "庭院门边",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_016a_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp", dim: true },
        { slot: "right", src: "./assets/sprites/a017/a017_teary.webp" }
      ],
      text: "A-017下意识碰了碰袖口，像那里也贴着一枚永远没机会再拍完的贴纸。她没说“我懂”，只把呼吸放轻——怕惊动门缝里任何还来得及被看见的东西。",
      pauseBefore: 200,
      pauseAfter: 240,
      textSpeed: 22
    },
    {
      location: "庭院门边",
      speaker: "旁白",
      voice: "./assets/voice/10-mizuki/10-mizuki_017_voice.wav",
      voiceReady: false,
      background: "./assets/bg/bg_garden_night.webp",
      sprites: [
        { slot: "left", src: "./assets/sprites/mizuki/mizuki_guard.webp", dim: true },
        { slot: "right", src: "./assets/sprites/a017/a017_stunned.webp" }
      ],
      text: "A-017望着她的背影，忽然明白，庭院不是由悲剧组成的。庭院是由“悲剧之后仍没能结束的动作”组成的。Azalea在确认，Meiling在清点，Shella在留位置，Mizuki在守望。"
    }
  ]
});
